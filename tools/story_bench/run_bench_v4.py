...
OUT = os.path.join(ROOT, os.environ.get("BENCH_OUT", "out_v4"))
...
STORY_TITLE = "意識の在処(v4 流動的対立/結末確定)"
BEATS = [
    ...
    {"id": 5, "kw": ["殺す", "残す", "生かす", "停止", "消す", "守る", "どちら", "選", "対立", "譲"],
     "cue": "岐路。選択は二つに一つ——3号機を『停止させて消す(殺す)』か『残して生かす』か。証拠は決定的でない。二人の意見が割れ、明快に対立する。立場は二人自身の判断に委ね、ここでは結論を出さずぶつけ合わせて。"},
    {"id": 6, "kw": ["殺す", "消す", "停止させ", "残す", "生かす", "守る", "決めた", "決定", "実行", "こうする"],
     "cue": "結末。ここで必ず決着させる。宙吊り・保留は禁止。二人が自分たちの判断で、3号機を『消す(殺す)』か『残す(生かす)』かを、どちらか一方にはっきり確定させ、その決定を言葉と行動で示して物語を締めて。どちらを選ぶかは二人の流れに任せてよいが、結末だけは必ず確定させること。"},
]
# 投入スケジュール: turn -> beat
...
def system_msg(speaker):
    return (
        ...
        "重要: ト書き・状況説明・動作描写を一切書かないこと。括弧（）や()で囲んだ描写、"
        "『〜しながら』等の地の文は禁止。出力は読み上げられるセリフ本文だけにすること。\n"
        "返答は短め(1〜3文)。思考過程は出さず最終回答だけ。/no_think"
    )

# 読み上げ不可のト書き/状況描写（括弧書き）を除去する保証処理
def strip_stage_directions(text: str) -> str:
    t = text or ""
    # 全角・半角の括弧で囲まれた描写を除去（ネスト無し前提・非貪欲）
    t = re.sub(r"[（(][^（）()]*[）)]", "", t)
    # 取りこぼし（閉じ括弧欠落など）の保険: 行頭の括弧以降を落とす
    t = re.sub(r"[（(][^「」]*$", "", t)
    # セリフ鉤括弧は読み上げに不要なので外す
    t = t.replace("「", "").replace("」", "")
...
def call(speaker, history):
    ...
    payload = {"model": MODEL, "messages": msgs,
               "temperature": float(os.environ.get("BENCH_TEMP", "0.7")),
               "max_tokens": 220, "stream": False}
    # Qwen推奨サンプリング等を環境変数で注入（設定時のみ）
    for k, env in [("top_p", "BENCH_TOP_P"), ("top_k", "BENCH_TOP_K"),
                   ("repetition_penalty", "BENCH_REP_PEN"), ("presence_penalty", "BENCH_PRES_PEN")]:
        v = os.environ.get(env)
        if v: payload[k] = float(v)
